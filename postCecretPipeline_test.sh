#!/bin/bash
version="postCecretPipeline version 1.2"
#title           :postCecretPipeline.sh
#description     :
#author		 :Jie.Lu@dshs.texas.gov  Karen.Bobier@dshs.texas.gov
#date            :20220803
...
sed 1d $result | awk -F '\t' '$2=="Undetermined" || $6=="fail" || $14<=20 || $26>=7500 {print $2}' - | sort > $result.failed
sed 1d $result | awk -F '\t' '$2=="Undetermined" || $6=="NA" || $14<=20 || $26>=7500 {print $2}' - | sort >> $result.failed
sed 1d $result | awk -F '\t' '{if($2!="Undetermined" && $6=="pass" && $14>20 && $26<7500){ print $2"\tComplete" } else {print $2"\tFailed"} }' - > $result.status
...
# 11-Pangolin_lineage	12-aligner_version 13-ivar_version 14-Reason_for_Sequencing	15-num_N
# 16-Pangolin_status
...
# Exclude Positive/Negative controls and Swab tests
awk -F '\t' '$3 ~ /Positive/ || $3 ~ /Negative/ || $3 ~ /Swab/ {print $1}' $result.demo.status.final | sort > $result.exclude
...
sed 1d $result | awk -F '\t' '$2!="Undetermined" && $6=="pass" && $14>20 && $26<7500 {print $2}' - | sort | grep -f $result.exclude -v - | sort > $result.filtered
# Colletion year
...
primer,"fastq",$17,$18,"","","",""}' $result.demo.status.sra | \
...
awk -F '\t' -v OFS='\t' '{ if($8=="pass" && $7>7500) $9="N>7500";print $0 } '|\
awk -F '\t' -v OFS='\t' '{ if ($3 ~ /Positive/ || $3 ~ /Negative/ || $3 ~ /Swab/) $4="Control";print $0}' |\
awk -F '\t' -v OFS='\t' '{ if ($3 ~ /Positive/ || $3 ~ /Negative/ || $3 ~ /Swab/) $9="";print $0}' \
...
zip -rj /home/dnalab/cecret_runs/zipfiles/postCecret_$1 $run_dir/$1* $run_dir/run_Cecret.*
aws s3 cp /home/dnalab/cecret_runs/zipfiles/postCecret_$1.zip s3://804609861260-covid-19/DATA/REPORT/postCecret_$1.zip
