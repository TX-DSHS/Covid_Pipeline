#!/bin/bash      
version="postCecretPipeline version 1.1"
...
sed 1d $result | awk -F '\t' '$2=="Undetermined" || $6=="fail" || $14<=20 || $26>=15000 {print $2}' - | sort > $result.failed
sed 1d $result | awk -F '\t' '$2=="Undetermined" || $6=="NA" || $14<=20 || $26>=15000 {print $2}' - | sort >> $result.failed
sed 1d $result | awk -F '\t' '{if($2!="Undetermined" && $6=="pass" && $14>20 && $26<15000){ print $2"\tComplete" } else {print $2"\tFailed"} }' - > $result.status
...
sed 1d $result | awk -F '\t' '$2!="Undetermined" && $6=="pass" && $14>20 && $26<15000 {print $2}' - | sort | grep -f $result.exclude -v - | sort > $result.filtered
# Colletion year
