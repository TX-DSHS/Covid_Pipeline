...
if [[ $(aws s3 ls s3://804609861260-covid-19/covid_sequencing_runs/$(aws s3 ls s3://804609861260-covid-19/covid_sequencing_runs/ |grep $sequencer |grep $run_date |cut -d " " -f29)Alignment_1/${folder1}Fastq/) ]]
  ...
  then
    seq_files=$(aws s3 ls  s3://804609861260-covid-19/covid_sequencing_runs/$(aws s3 ls s3://804609861260-covid-19/covid_sequencing_runs/ |grep $sequencer |grep $run_date |cut -d " " -f29)Data/Intensities/BaseCalls/ |grep "TX-DSHS" |awk '{print $4}')
    for i in $seq_files
      do
