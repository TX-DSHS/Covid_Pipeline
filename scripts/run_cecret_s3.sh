...
if (sequencer="M06018")
  then
    aws s3 sync s3://804609861260-covid-19/covid_sequencing_runs/${run_date}_${sequencer}*/Alignment_1/20*/Fastq/ $basedir/reads/ --exclude "*" --include TX*.fastq.gz  #this sequencer has a different file structure for output
  else
    aws s3 sync s3://804609861260-covid-19/covid_sequencing_runs/${run_date}_${sequencer}*/Data/Intensities/BaseCalls/ $basedir/reads/ --exclude "*" --include  TX*.fastq.gz 
fi
...
cd $basedir
#run cecret
/work/software/nextflow run $basedir/Cecret/Cecret_DSHS.nf -c $basedir/Cecret/configs/docker.config
#rename sumamry file
...
aws s3 cp  /home/dnalab/cecret_runs/zipfiles/${1}.zip s3://804609861260-covid-19/covid_sequencing_runs/cecret_runs/zip_files/
aws s3 cp  /home/dnalab/cecret_runs/zipfiles/ALl_to_${1}.zip s3://804609861260-covid-19/covid_sequencing_runs/cecret_runs/zip_files/

aws s3 cp $basedir/run_results_$1.txt s3://804609861260-covid-19/covid_sequencing_runs/cecret_runs/run_results/
